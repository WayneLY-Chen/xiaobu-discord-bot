...
> **目前進度：Phase 6 已完成（全部階段結束）。** 聊天、多伺服器隔離、多 AI Provider 與自動換手、網路搜尋、天氣、計算機、長期記憶、伺服器背景知識、AI 生圖、語音對話都可實際使用。YouTube 音樂已決定不做 —— 詳見下方「功能狀態」。
...
資料庫建好的 `music_queues` 資料表目前沒有用到（Phase 5 取消）。
...
Whisper 對 `language=zh` **一律輸出簡體**，而且支援清單裡沒有 `zh-TW`。官方建議用 `prompt` 參數偏置輸出，但實測 **Groq 的端點只要 prompt 帶中文就回 HTTP 500**（turbo 與 large-v3 都試過），英文 prompt 則完全沒效果。

所以在自己這端用 OpenCC 轉，而且用 `twp` 而不是 `tw`：

| 模式 | 結果 |
|---|---|
| `tw` | 這個**軟件**的**默認設置**需要重新配置**網絡連接** |
| `twp` | 這個**軟體**的**預設設定**需要重新配置**網路連線** ✅ |

`tw` 只換字形，`twp` 連詞彙一起換成台灣用法。

另外會過濾掉 Whisper 對著靜音常見的「幻聽」—— 那些字幕組署名（`字幕由Amara.org社群提供` 之類）在它的訓練資料裡大量存在，對著雜訊很容易吐出來，不該送進 AI。
